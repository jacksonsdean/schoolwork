...
class Game:
    ...
    def update_canvas(self):
        """This function is called once per tick and updates every moving object on the canvas, and then redraws the canvas and checks for user events"""
        if not Game.paused:
            Game.ticks +=1  
            time.sleep(.005)
            if len(Game.items)==0:
