...
class Game:
    def __init__(self,master=""):
        ...
        restart_button=Button(Game.infobar, text="RESTART", command=restart)
        restart_button.grid(row = 1, column = 4)
        quit_button = Button(Game.infobar, text="QUIT", command=quit)
        quit_button.grid(row = 1, column = 5)
        
        
        #setup enemies--------------------------
        #enemy1 = Squirrel(self,x=600,y=200,speedX=0,speedY = 2.3, shotFreq = 100)
...
class Player:
    def __init__(self, parent):
        ...
        self.scot = Game.c.create_image(18,275,image=self.scot_r) 
    ...
    #COLLISION-------------------------------------------------------
        ...
        for key in Game.walls.keys():
            ...
            if abs(playerx - wallX2)<=13:
                if wallY1<= playery-22<= wallY2 or wallY1<= playery+22<= wallY2 or wallY1<= playery<= wallY2:
                    left_collision=True
                    col = True
            #RIGHT      
            ...
            if abs(playery - wallY2)<=25:
                if wallX1<= playerx-8 <= wallX2 or wallX1<=playerx+8 <=wallX2:
                    up_collision=True
                    col = True
        ...
        #ITEM COLLISION-------------------------
            ...
            if abs(playerx - kbx[2])<=13:
                if kbx[1]<= playery-20 <= kbx[3] or kbx[1]<=playery+35 <= kbx[3]:
                    key_pickup = True 
            #RIGHT      
            ...
            if abs(playery - kbx[3])<=25:
                if kbx[0]<= playerx-6 <= kbx[2] or kbx[0]<=playerx+6 <=kbx[2]:
                    key_pickup = True 
            #DOWN        
...
class Turret:
    def __init__(self,x,y,fireRate):
        self.obj = Game.c.create_rectangle(x-8, y-8, x+8, y+8,fill="red")
