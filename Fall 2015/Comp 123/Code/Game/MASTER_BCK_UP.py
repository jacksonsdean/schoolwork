...
class Player:
    def __init__(self, parent):
        ...
        self.hitbox = Game.c.create_rectangle(3,250,32,315,fill='',outline='black')
        self.scot = Game.c.create_image(18,275,image=self.scot_r,tag = self) 
...
#MOVE----------------------------------------------------------------------------------
        ...
        elif self.dirs["LEFT"] and not(self.left_collision):
            Game.c.move(self.scot, -5, 0); Game.c.move(self.hitbox, -5, 0)
        elif self.dirs["UP"] and not(self.up_collision):
            Game.c.move(self.scot, 0, -5); Game.c.move(self.hitbox, 0, -5)
        elif self.dirs["DOWN"] and not(self.down_collision):
            Game.c.move(self.scot, 0, 5); Game.c.move(self.hitbox, 0, 5)
                
                
        elif self.dirs["RIGHT"] and self.dirs["UP"] and not(self.right_collision or self.up_collision):
            Game.c.move(self.scot, 5, 5); Game.c.move(self.hitbox, 5, 5)
        elif self.dirs["LEFT"] and self.dirs["UP"] and not(self.left_collision or self.up_collision):
            Game.c.move(self.scot, -5, 5); Game.c.move(self.hitbox, -5, 5)
        elif self.dirs["RIGHT"] and self.dirs["DOWN"] and not(self.down_collision or self.right_collision):
            Game.c.move(self.scot, 5, -5); Game.c.move(self.hitbox, 5, -5)
        elif self.dirs["LEFT"] and self.dirs["DOWN"] and not(self.down_collision or self.left_collision):
    ...
    def die(self):
        ...
        Game.c.coords(self.hitbox,18,275)
...
class Turret:
    ...
    def update(self):
        ...
        if self.n%self.fireRate == 0:
            shot = Projectile(Game, "popcorn",x,y,0,-1)
